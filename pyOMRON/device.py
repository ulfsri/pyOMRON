...
class Omron(ABC):
    ...
    def __prepend(self, frame: list) -> list:
        ...
        """
        return ['\x02', '\x30', '\x31', '\x30', '\x30', '\x30'] + frame # [STX, Unit No., Unit No., Sub-address, Sub-addres, SID]
    ...
    async def variable_area_write(self, command: str, set_values: float):
        ...
        """
        command_data = []
        for i, c in enumerate(command): # Convert every character of the command to a byte
             command_data.append(c) # Add each byte to the command_data list
        no_elements = ['\x30', '\x30', '\x30', '\x31'] # Writes to 1 element per call
        byte_list = ['\x30', '\x31', '\x30', '\x32'] + command_data + ['\x30', '\x30'] # [MRC, MRC, SRC, SRC, command, Bit Position, Bit Position]
        for i, c in enumerate(no_elements):
            byte_list.append(c)
        if ((command[1] == 'E' and (int(command[5]) != 6 or int(command[4:5]) != 14)) or (command[1] == '1' and int(command[5], 16) != 14 )): # Checks for 'Status', 'Version', or 'Heater Burnout Threshold'
            set_values = int(float(set_values)*10) # Otherwise makes data ten times actual value
        set_values = hex(set_values) 
        set_values = set_values[set_values.index('x')+1:].upper() # Removes '0x' from the hex value and makes it uppercase
        ...
        byte += bytes([self.__bcc_calc(byte_list)]) # Adds bcc to the end of the command
        ret = await self._device._write_readline(byte) # Writes the command and reads the response
        ret = ret.hex()
        self.__end_code(ret)
        if ret[23] != '0' or ret[25] != '0'  or ret[27] != '0' or ret[29] != '0': # Checks response code, '0000' is 'Normal Completion'
            print("Error occured")
            if ret[23] == '1' and ret[25] == '0'  and ret[27] == '0' and ret[29] == '1':
                print("Command length too long")
            elif ret[23] == '1' and ret[25] == '0'  and ret[27] == '0' and ret[29] == '2':
                print("Command length too short")
            elif ret[23] == '1' and ret[25] == '1'  and ret[27] == '0' and ret[29] == '1':
                print("Area Type Error")
            elif ret[23] == '1' and ret[25] == '0'  and ret[27] == '0' and ret[29] == '3':
                print("Number of elements/Number of data do not agree")
            elif ret[23] == '1' and ret[25] == '1'  and ret[27] == '0' and ret[29] == '0':
                print("Parameter error")
            elif ret[23] == '2' and ret[25] == '2'  and ret[27] == '0' and ret[29] == '3':
            ...
            for i in range(int(byte[8:12])):
                ...
                print(f"{self.addresses[byte[0:2]][list(self.addresses[byte[0:2]])[list(self.addresses[byte[0:2]]).index(byte[2:6])+i]]} set")
    ...
    async def variable_area_read(self, command: str):
        ...
        for i, c in enumerate(command): # Convert every character of the command to a byte
             command_data.append(c) # Add each byte to the command_data list
        byte_list = ['\x30', '\x31', '\x30', '\x31'] + command_data + ['\x30', '\x30'] # [MRC, MRC, SRC, SRC, command, Bit Position, Bit Position]
        ...
        if ret[23] != '0' or ret[25] != '0'  or ret[27] != '0' or ret[29] != '0': # Checks response code, '0000' is 'Normal Completion'
            ...
            if ret[23] == '1' and ret[25] == '0'  and ret[27] == '0' and ret[29] == '1':
                ...
                print("Area Type Error")
            elif ret[23] == '1' and ret[25] == '1'  and ret[27] == '0' and ret[29] == 'B':
            ...
            for i in range(int(byte[8:12])): # For each element in response (should be one)
                if byte[0:1] == 'C': # The eight bit case
                    print_dict[self.addresses[byte[0:2]][list(self.addresses[byte[0:2]])[list(self.addresses[byte[0:2]]).index(byte[2:6])+i]]] = bytes.fromhex(ret[0+16*i:16+16*i]).decode("ascii")
                elif byte[0:1] == '8': # The 4 bit case
                    print_dict[self.addresses[byte[0:2]][list(self.addresses[byte[0:2]])[list(self.addresses[byte[0:2]]).index(byte[2:6])+i]]] = bytes.fromhex(ret[0+8*i:8+8*i]).decode("ascii")
                else:
        ...
        for c in comm:
            for var_type, dict in self.addresses.items():
                for add, command in dict.items():
                    if c == command:
                        comm_add = var_type + add # Search through addresses to find the address for the comm
            ret_dict .update(await self.variable_area_read(comm_add)) # Adds the result to the dictionary
        return ret_dict
